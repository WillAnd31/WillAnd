import {Interest} from './../models/interest.model';

export let Interests: Interest[] = [
	new Interest(
		'BE_WEB_DEV',
		'https://jvzoohost.com/img/jvZoo-images/solutions-server.png'
	),
	new Interest(
		'FE_WEB_DEV',
		'https://devcodecamp.com/wp-content/uploads/2016/05/html-css-js.png'
	),
	new Interest(
		'APPLE',
		'https://upload.wikimedia.org/wikipedia/commons/thumb/f/fa/Apple_logo_black.svg/2000px-Apple_logo_black.svg.png'
	),
	new Interest(
		'IOS_DEV',
		'https://retrocdn.net/images/thumb/f/f9/Logo-ios.svg/320px-Logo-ios.svg.png'
	),
	new Interest(
		'SPACEX',
		'https://iclothproducts.com/wp-content/uploads/2016/01/SpaceX-Logo-1024x261-1.png'
	),
	new Interest(
		'TESLA',
		'https://upload.wikimedia.org/wikipedia/en/thumb/6/68/Tesla_Motors_logo.svg/346px-Tesla_Motors_logo.svg.png'
	),
	new Interest(
		'TECH',
		'https://cdn4.iconfinder.com/data/icons/stash/128/computer-chip-512.png'
	),
	new Interest(
		'TV_SHOWS',
		'https://pbs.twimg.com/profile_images/687320361201987584/4UdE2wWe.jpg'
	),
	new Interest(
		'PROGRAMMING',
		'https://cdn4.iconfinder.com/data/icons/small-n-flat/24/terminal-512.png'
	),
	new Interest(
		'MUSIC',
		'https://upload.wikimedia.org/wikipedia/en/thumb/6/65/RHbear.svg/1011px-RHbear.svg.png'
	),
	new Interest(
		'BASEBALL',
		'https://upload.wikimedia.org/wikipedia/commons/thumb/9/92/Baseball.svg/2000px-Baseball.svg.png'
	),
	new Interest(
		'BOOKS',
		'https://findicons.com/files/icons/490/mixed/512/book.png'
	)
];
